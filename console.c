...
int dividedby(int dividend, int divisor){
  int quotient = 0;
  while(dividend>0){
...
int exponentiation(char* arr[100], int size){
  ...
  int power,base,temp;
  size--;
  if(isIntNumber(arr[size-1])!=0){                       // checks if the last operand is a number
    temp=atoi(arr[n]);
    if(arr[n][0]=='-'){
      temp=temp-(temp*2);
    }
  ...
  if(isIntNumber(arr[size-2])!=0){                       // checks if the second to the last operand is a number
    temp=atoi(arr[n]);
    if(arr[n][0]=='-'){
      temp=temp-(temp*2);
    }
...
int console_execute(const char *str){
   ...
   if (strcmp(u,"arith") == 0){ //-- Solves input integer based on arithmetic operations. Args: [-operation] arguments
      int result=0,operation = 0, size = 0;
      char v[20];
      char* argsArr[100];
      argsArr[size] = "arith";
      size++;
      ...
      result=arithFunction(argsArr,size+1);
   }else    
   if (strcmp(u,"stat") == 0){ //-- Solves input integer based on statistical operations. Args: [-operation] arguments 
      int result=0,operation = 0, size = 0;
      char v[20];
      char* argsArr[100];
      argsArr[size] = "stat";
      ...
      result=statFunction(argsArr,size+1);
   }else 
   if(strcmp(u,"min")==0){  //-- Returns the minimum value from the input integers
      int result=0, size = 0;
      char v[20];
      char* argsArr[100];
      argsArr[size] = "min";
      ...
      result=minFunction(argsArr,size+1);
   }else 
   if(strcmp(u,"max")==0){  //-- Returns the maximum value from the input integers
      int result=0, size = 0;
      char v[20];
      char* argsArr[100];
      argsArr[size] = "max";
      ...
      result=maxFunction(argsArr,size+1);
   // }else if(strcmp(u, "cut")==0){
   //    char* argsArr[100];
   //    strcpy(argsArr[0], "mv");
